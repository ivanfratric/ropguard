...
int IsAddressExecutable(LPCVOID address) {
	if(GetROPSettings()->executableModuleCache) {
		...
		for(unsigned int i=0; i<executableModules->size(); i++) {
			if((((unsigned long)address)>=(*executableModules)[i].start)&&(((unsigned long)address)<(*executableModules)[i].end)) {
	...
	DWORD exeProtect = PAGE_EXECUTE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE | PAGE_EXECUTE_WRITECOPY;
	MEMORY_BASIC_INFORMATION info;
	memset(&info,0,sizeof(MEMORY_BASIC_INFORMATION));
	if(!VirtualQuery(address,&info,sizeof(info))) return 0;
	if(info.Protect & exeProtect) {
		if(GetROPSettings()->executableModuleCache) {
...
int PrecededByCall(unsigned char *address) {
	...
	if(*(address-3) == 0xE8) return 1;
	if(*(address-5) == 0x9A) return 1;
...
int CheckCallTarget(unsigned char *returnAddress, unsigned long *registers, unsigned long cfAddress) {
	int i;
	...
	if(*(returnAddress-5) == 0xE8) {
		...
		if(callAddress == (cfAddress^ADDR_SCRAMBLE_KEY)) return 1;
	...
	//direct far call
	if(GetROPSettings()->allowFarCFCalls) {
		if(*(returnAddress-5) == 0x9A) return 1;
		if(*(returnAddress-7) == 0x9A) return 1;
	}
	...
	if(GetROPSettings()->allowIndirectCFCalls) {
		...
		if(GetROPSettings()->allowFarCFCalls) {
			for(i=2;i<10;i++) {
				if((*(returnAddress-i) == 0xFF) && (((*(returnAddress-i+1))&0x38)==0x18)) return 1;
			}
		}
	}
...
int CFCheckReturnAddress(DWORD returnAddress, DWORD functionAddress, DWORD *registers) {
	...
	//is the target of call instruction preceding the return address the same as the address of critical function?
	if((GetROPSettings()->checkCallTarget)&&(!CheckCallTarget((unsigned char *)returnAddress, registers, functionAddress))) {
		stringstream errorreport;
...
int SimulateProgramFlow(unsigned long stackPtr, unsigned long stackIncrement) {
	...
	n = GetROPSettings()->maxInstructionsToSimulate;
	...
	for(i=0;i<n;i++) {
		//printf("%x %x\n",eip,stackPtr); getch();
		simret = SimulateStackInstruction(NULL, &eip, &stackPtr, &stackIncrement);
		if(!simret) break;
		if(simret == 2) { //we reached RETN instruction, check the return address
			returnAddress = *((unsigned long *)stackPtr);
...
void __stdcall ROPCheck(unsigned long functionAddress, unsigned long *registers) {
	if(!protectionEnabled) return; //still initializing protection
	...
	if(!currentFunction) {
		MessageBoxA(NULL, "Inside ROPCheck, but guarded function not identified", "ROPGuard", MB_OK);
		return;
	}
	...
	if(GetROPSettings()->executableModuleCache) {
		...
		if(!currentFunction->criticalFunction) return;
	}
	...
	if(GetROPSettings()->checkFunctionAddressOnStack) {
		if(!CheckFunctionAddressOnStack(functionAddress, (unsigned long *)stackPointer)) return;
	}

	if(GetROPSettings()->checkStackPointer) {
		if(!CheckStackPointer(stackPointer + GetROPSettings()->preserveStack)) return;
	}

	if(GetROPSettings()->checkReturnAddress) {
		DWORD returnAddress = *((DWORD *)(stackPointer + GetROPSettings()->preserveStack));
		if(!CFCheckReturnAddress(returnAddress, functionAddress, registers)) return;
		//if(!CheckReturnAddress(returnAddress)) return;
	}

	if(GetROPSettings()->checkStackFrames) {
		if(!CheckStackFrames((DWORD *)(stackPointer + GetROPSettings()->preserveStack), (DWORD *)framePointer)) return;
	}
	...
	//function-speciffic protections

	//prevent loading a library over smb
	if((GetROPSettings()->preventLoadLibraryFromSMB) && (strcmp(currentFunction->functionName,"LoadLibraryExW")==0)) {
		...
		if((dllname[0] == '\\')&&(dllname[2] == '\\')) {
			if(!((dllname[4] == '?')&&(dllname[6] == '\\'))) {
				...
				errorreport << "Stack ponter: " << std::hex << stackPointer << "\n";
	...
	//prevent changing access rigts of stack
	if((GetROPSettings()->preventVirtualProtectOnStack) && (strcmp(currentFunction->functionName,"VirtualProtect")==0)) {
		DWORD address = *((DWORD *)(stackPointer + GetROPSettings()->preserveStack + 4));
	...
	if((GetROPSettings()->preventVirtualProtectOnStack) && (strcmp(currentFunction->functionName,"VirtualProtectEx")==0)) {
		DWORD address = *((DWORD *)(stackPointer + GetROPSettings()->preserveStack + 8));
		DWORD stackBottom, stackTop;
		GetStackInfo(&stackBottom, &stackTop);
		if((address>=stackBottom)&&(address<stackTop)) {
			stringstream errorreport;
